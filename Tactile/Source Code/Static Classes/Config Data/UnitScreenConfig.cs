    ...
    class UnitScreenConfig
    {
        ...
        public readonly static UnitScreenData[] UNIT_DATA = new UnitScreenData[]
        {
            ...
            #region Page 2 - Primary Stats
                new UnitScreenData(0, "Str", 1,
                    function: (object input, int page) => ((Game_Unit)input).stat(Stat_Labels.Str),
                    dataOffset: 16,
                    textColor: (object input) => ((Game_Unit)input).actor.get_capped(Stat_Labels.Str) ? "Green" : "Blue"),
                new UnitScreenData(20, "Mag", 1,
                    function: (object input, int page) => ((Game_Unit)input).stat(Stat_Labels.Mag),
                    dataOffset: 16,
                    textColor: (object input) => ((Game_Unit)input).actor.get_capped(Stat_Labels.Mag) ? "Green" : "Blue"),
                new UnitScreenData(40, "Dex", 1,
